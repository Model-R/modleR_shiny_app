...
# IMPORTANT -------------------------
# In Mac OS X, to correctly load the rJava package please include the commands commented bellow
...
function(input, output, session) {
  ...
  dismo.mod <- function(sp,
    ...
    isolate( {
      ...
      # Extracting values from variables at record points
      presvals <- raster::extract(var, coord)
      
      ## Setting seed to always have the same random points
      set.seed(seed)
      
      ## Generating random pseudo-absences 
      backgr <- randomPoints(var, numpontos)
      
      ## Names of the coordinate columns for the background points
      colnames(backgr) = c("Longitude", "Latitude")
      
      ## Extracting values from variables at background points
      absvals <- raster::extract(var, backgr)
      
      ## Creating a '1' and '0' vector corresponding to presence and absence points respectively.
      ...
      for (i in unique(group_pre)) {
        progress$set(message = paste("Processing modeling...", i), value = 0)
        ...
        envtest_pre <- subset(sdmdata_teste, pre_abs == 1, select = c(-group, -Longitude, 
          -Latitude, -pre_abs))
        ...
        if (GLM == T) {
          ...
          # Transforma em binario o modelo continuo cortando-o pelo limiar
        ...
        ## Modelos continuos 
        if (write.cont == T) {
          cat(paste("Saving continuous models...", sp, i, "\n"))
  ...
  modelagem <- function() ({
    ...
    dismo.mod(
      "", especie, pred_nf, pred_nf2, input$MAXENT, input$BIOCLIM, input$GLM,
      ...
      if (file.exists(paste0("www/", projeto, "/final/mx_ensemble.tif"))) {
        ...
        if (file.exists(paste0("www/", projeto, "/final/proj_ensemble.tif"))) {
          map <- leaflet() %>%
            ...
            addRectangles(
              ext1, ext3, ext2, ext4, color = "green", fill = FALSE,
      ...
      if (file.exists(paste0("www/", projeto, "/final/svm_ensemble.tif"))) {
        ...
        if (file.exists(paste0("www/", projeto, "/final/proj_ensemble.tif"))) {
          map <- leaflet() %>%
            ...
            addRasterImage(rproj, colors = pal, opacity = 0.8) %>%
            addLegend(pal = pal, values = values(r), title = "SVM") %>%
            addCircles(
              color = "red",
              lat = especie[, 2], lng = especie[, 1]
            ) %>% # addMarkers(especie[,1], especie[,2]) %>%
            addRectangles(
              ext1, ext3, ext2, ext4, color = "green", fill = FALSE,
              dashArray = "5,5", weight = 3
      ...
      if (file.exists(paste0("www/", projeto, "/final/ma_ensemble.tif"))) {
        ...
        if (file.exists(paste0("www/", projeto, "/final/proj_ensemble.tif"))) {
          map <- leaflet() %>%
            ...
            addRasterImage(rproj, colors = pal, opacity = 0.8) %>%
            addLegend(pal = pal, values = values(r), title = "Maha") %>%
            addCircles(
              color = "red",
              lat = especie[, 2], lng = especie[, 1]
            ) %>% # addMarkers(especie[,1], especie[,2]) %>%
      ...
      if (file.exists(paste0("www/", projeto, "/final/rf_ensemble.tif"))) {
        ...
        if (file.exists(paste0("www/", projeto, "/final/proj_ensemble.tif"))) {
          map <- leaflet() %>%
            ...
            addRasterImage(rproj, colors = pal, opacity = 0.8) %>%
            addLegend(pal = pal, values = values(r), title = "RF") %>%
            addCircles(
              color = "red",
              lat = especie[, 2], lng = especie[, 1]
            ) %>% # addMarkers(especie[,1], especie[,2]) %>%
      ...
      if (file.exists(paste0("www/", projeto, "/final/glm_ensemble.tif"))) {
        ...
        if (file.exists(paste0("www/", projeto, "/final/proj_ensemble.tif"))) {
          map <- leaflet() %>%
            ...
            addRasterImage(rproj, colors = pal, opacity = 0.8) %>%
            addLegend(pal = pal, values = values(r), title = "GLM") %>%
            addCircles(
              color = "red",
              lat = especie[, 2], lng = especie[, 1]
            ) %>% # addMarkers(especie[,1], especie[,2]) %>%
    ...
    output$uiarquivosmodelos <- renderUI({
      lista_models <- list.files(
        paste0("www/", projeto, "/models"), full.names = F,
        pattern = paste0("pre_")
      )
    ...
    output$ui <- renderUI({
      lista_jpg <- list.files(
        paste0("www/", projeto, "/jpg"), full.names = F,
        pattern = paste0(".jpg")
      )
    ...
    output$uiarquivosdados <- renderUI({
      lista_csv <- list.files(
        paste0("www/", projeto, "/csv"), full.names = F,
        pattern = paste0(".csv")
      )
    ...
    output$uiarquivosensemble <- renderUI({
      lista_final <- list.files(
        paste0("www/", projeto, "/final"), full.names = F,
        pattern = paste0(".tif")
      )
    ...
    output$uiarquivosprojecao <- renderUI({
      lista_proj <- list.files(
        paste0("www/", projeto, "/proj"), full.names = F,
        pattern = paste0(".tif")
      )
    ...
    output$uiarquivosprojecaofuturo <- renderUI({
      lista_futuro <- list.files(
        paste0("www/", projeto, "futuro"), full.names = F,
        pattern = paste0(".tif")
      )
  ...
  output$mapapontosextend2 <- renderLeaflet({
    if (!is.null(especie)) {
      ...
      map <- leaflet() %>% addTiles() %>% addRectangles(
    ...
    withProgress(message = "", value = 0, {
      ...
      isolate({
        ...
        if (input$tipodadoabiotico == "BIOORACLE") {
          ...
          pathfuturo <- paste(
            getwd(), "/ex/biooracle/", input$periodobiooracle,
  ...
  output$mapadistribuicaodatacleaning <- renderLeaflet({
    ...
    if (!is.null(especie)) {
      ...
      showModal(modalDialog(
        title = "Warning!", "Please inform occurrence data",
        easyClose = TRUE
      ))
    }
  })
  ...
  output$mapadistribuicao <- renderLeaflet({
    ...
    if (!is.null(especie)) {
      if (input$tipodado == "gbif") {
        ...
        if (input$tipodado == "csv") {
          map <- leaflet(especie) %>% addTiles() %>% addCircles(
            color = "red",
  ...
  observeEvent(input$btnconsultarprojeto, {
    ...
    if (projeto != "projeto/") {
      if (file.exists(paste0(getwd(), "/www/", projeto)) == TRUE) {
        ...
        output$uifinal <- renderUI({
          ...
          lapply(1:length(sort(lista_modelsfinal)), function(i) {
            tags$a(
              ...
              tags$img(
                src = paste0(projeto, "/final/", lista_modelsfinal[i]),
                height = "200px"
              ), target = "_blank"
            )
          })
        ...
        output$uiscript <- renderUI({
          lista_txt <- list.files(
            paste0("www/", projeto, "/"), full.names = F,
      ...
      if (file.exists(paste0(getwd(), "/www/", projeto)) == FALSE) {
        showModal(modalDialog(
          title = "Invalid Id!", paste0(
            "Project '", input$edtprojeto,
            "' was not found."
          ), br(), paste0("Please enter a registed Id!"),
          easyClose = TRUE
        ))
      }
    }
