...
getOcorrencia <- function(pTaxon) {
  ...
  json_file <- paste0("http://aplicacoes.jbrj.gov.br/jabot/v2/ws/server.php?coordenada=S&taxon=", 
                      pTaxon)
  ...
  y2 <- cbind(as.numeric(y2[, 2]), as.numeric(y2[, 3]))
...
function(input, output, session) {
  ...
  dismo.mod <- function(sp,
                        ...
                        numpontos = 500,
                        seed=123,
                        write.cont=T, # escreve modelos continuos
                        bin=T, # faz modelos binrios
                        write.bin=T, # escreve modelos binarios
                        mult=T, # faz modelos multiplicados(bin*con)
                        write.mult=T, # escreve modelos multiplicados
                        TSS.value = 0.2,
                        future.model=F, # faz modelos futuros
                        future.raster=newdata, # variaveis futuras
    ...
    isolate( {
      ...
      coord <- especie
      n <- nrow(coord)
      ## Extraindo os valores das variaveis onde ha pontos de registros
      presvals <- raster::extract(var, coord)
      
      ## Seeting seed para sempre criar os mesmos pontos aleatarios
      set.seed(seed)
      
      ## Gerando as pseudoausancias aleatoriamente
      ...
      ## Extraindo os valores das variaveis onde ha pseudoausencias
      absvals <- raster::extract(var, backgr)
      
      ## Cria um vetor contendo algarismo '1' e '0' correspondendo ao numero de
      ## registros presencas e ausencias respectivamente.
      ...
      ## Setting seed para distribuir as presencas sempre para as mesmas partcoes
      set.seed(seed)
      
      ## Separando as presencas e pseudoausencias nos grupos das partcoes
      ...
      for (i in unique(group_pre)) {
        ...
        # Preparando sdmdata test para avaliar modelos que nao usam so presenca
        envtest_pre <- subset(sdmdata_teste, pre_abs == 1, select = c(-group, -Latitude, 
                                                                      -Latitude, -pre_abs))
        envtest_abs <- subset(sdmdata_teste, pre_abs == 0, select = c(-group, -Latitude, 
                                                                      -Latitude, -pre_abs))
        
        ### Separando os dados (apenas coordenadas) para fazer BioClim, Maxent e
        ### Mahalanobis que usam so presenca
        coord_pres_train <- subset(sdmdata_train, pre_abs == 1, select = c(Longitude, 
                                                                           Latitude))
        coord_pres_teste <- subset(sdmdata_teste, pre_abs == 1, select = c(Longitude, 
                                                                           Latitude))
        coord_abs_train <- subset(sdmdata_train, pre_abs == 0, select = c(Longitude, 
                                                                          Latitude))
        coord_abs_teste <- subset(sdmdata_teste, pre_abs == 0, select = c(Longitude, 
                                                                          Latitude))
        ...
        if (Bioclim == T) {
          ...
          ebc <<- dismo::evaluate(coord_pres_teste, coord_abs_teste, bc, 
                                  var)
          # Calculo do TSS
        ...
        ### SALVA ARQUIVOS DE Validacao DE PERFORMANCE
        cat(paste("Saving validation file...", sp, i, "\n"))
        sink(file = paste0("./www/", projeto, "/models/evaluate_", sp, ".txt"), 
             split = T, append = T)
        if (Bioclim == T) {
        ...
        # cat(paste('Salvando arquivos de validação para todas as
        # espécies...',sp,i,'\n'))
        sink(file = paste0("./www/", projeto, "/models/evaluate_ALL_models.txt"), 
             split = T, append = T)
        if (Bioclim == T) {
          print(paste(sp, spname, i, "BioClim", round(ebc@auc, 3), round(bcTSS, 
                                                                         3), round(tbc, 3), round(threshold(ebc)$kappa, 3), round(threshold(ebc)$equal_sens_spec, 
      ...
      ensemble_arquivos <- list.files(paste0("./www/", projeto, "/final/"), full.names = T, 
  ...
  modelagem <- function() ({
    ...
    dismo.mod("", especie, pred_nf, pred_nf2, input$MAXENT, input$BIOCLIM, input$GLM, 
              input$RF, input$SVM, input$MAHALANOBIS, input$DOMAIN, input$SVM2, numparticoes, 
              numpontos, 123, T, T, T, F, F, input$TSS, futuro, pred_nffuturo, futuro, 
              write.projecao)
    ...
    # verificando se foi gerado o arquivo de projeto ensemble final gero o arquivo
    # raster para ser colocado no mapa
        ...
        if (file.exists(paste0("www/", projeto, "/final/proj_ensemble.tif"))) {
          map = leaflet() %>% addTiles %>% addRasterImage(r, colors = pal, 
            ...
            addLegend(pal = pal, values = values(r), title = "SVM") %>% addCircles(color = "red", 
                                                                                   lat = especie[, 2], lng = especie[, 1]) %>% # addMarkers(especie[,1], especie[,2]) %>%
            addRectangles(ext1, ext3, ext2, ext4, color = "green", fill = FALSE, 
                          dashArray = "5,5", weight = 3)
        } else {
          map = leaflet() %>% addTiles %>% addRasterImage(r, colors = pal, 
  ...
  output$mapapontosextend <- renderLeaflet({
    if (!is.null(especie)) {
      ...
      map = leaflet(especie) %>% addTiles %>% addMarkers(clusterOptions = markerClusterOptions()) %>% 
        addMarkers(~Longitude, ~Latitude) %>% addRectangles(input$edtextend1, 
  ...
  output$mapapontosextend2 <- renderLeaflet({
    if (!is.null(especie)) {
      ...
      map = leaflet() %>% addTiles %>% addRectangles(input$edtextend12, input$edtextend32, 
    ...
    withProgress(message = "", value = 0, {
      ...
      isolate({
        if (input$tipodadoabiotico == "CLIMA") 
        {
          ...
          pathfuturo = paste0(getwd(), "/ex/clima/", input$periodo, "/", 
          ...
          if (input$resolucao == "30s") {
            ...
            if (any(input$Bio1, input$Bio2, input$Bio3, input$Bio4, input$Bio5, 
              ...
              file.rename(from = files, to = sub(pattern = "bio_", replacement = "bio", 
                                                 files))
            }
            
            if (any(input$Bio10, input$Bio11, input$Bio12, input$Bio13, input$Bio14, 
          ...
          if (input$periodo != "current") {
            ...
            files.tif <- paste0(pathfuturo, "/", input$gcm, input$rcp, "bi", 
                                year, 1, ".tif")
            for (i in c(2:19)) {
              add <- paste0(pathfuturo, "/", input$gcm, input$rcp, "bi", 
                            year, i, ".tif")
            ...
            if (any(file.exists(files.tif) != T)) {
              ...
              if (input$resolucao == "2-5m") {
                url = paste0("http://biogeo.ucdavis.edu/data/climate/cmip5/", 
                             "2_5m", "/", zip_future)
              }
              if (input$resolucao != "2-5m") {
                url = paste0("http://biogeo.ucdavis.edu/data/climate/cmip5/", 
            ...
            if (input$Bio1 == TRUE) {
              arquivofuturo <- c(arquivofuturo, paste0(pathfuturo, "/", input$gcm, 
        ...
        if (input$tipodadoabiotico == "BIOORACLE") 
        {
          ...
          ############################################################################# SE FOI ESCOLHIDO ALGUM PERÃ\u008dODO DIFERENTE DO CURRENT ENTAO PROJETO O
          ############################################################################# FUTURO
          if (input$periodobiooracle != "current") 
          {
            if (input$sstmin == TRUE) {
              arquivofuturo <- c(arquivofuturo, load_layers(paste0("BO_", 
                                                                   input$cenariobiooracle, "_", input$periodobiooracle, "_", 
                                                                   "sstmin"), rasterstack = FALSE, datadir = pathfuturo))
            }
            if (input$sstmax == TRUE) {
              arquivofuturo <- c(arquivofuturo, load_layers(paste0("BO_", 
                                                                   input$cenariobiooracle, "_", input$periodobiooracle, "_", 
                                                                   "sstmax"), rasterstack = FALSE, datadir = pathfuturo))
            }
            if (input$sstrange == TRUE) {
              arquivofuturo <- c(arquivofuturo, load_layers(paste0("BO_", 
                                                                   input$cenariobiooracle, "_", input$periodobiooracle, "_", 
                                                                   "sstrange"), rasterstack = FALSE, datadir = pathfuturo))
            }
            if (input$salinity == TRUE) {
              arquivofuturo <- c(arquivofuturo, load_layers(paste0("BO_", 
                                                                   input$cenariobiooracle, "_", input$periodobiooracle, "_", 
                                                                   "salinity"), rasterstack = FALSE, datadir = pathfuturo))
            }
            if (input$sstmean == TRUE) {
              arquivofuturo <- c(arquivofuturo, load_layers(paste0("BO_", 
                                                                   input$cenariobiooracle, "_", input$periodobiooracle, "_", 
                                                                   "sstmean"), rasterstack = FALSE, datadir = pathfuturo))
            }
          }  # IF CURRENT
        ...
        if (length(arquivo) > 0) {
          if ((selecionado == TRUE) && (exists("especie"))) {
            ...
            # Select 300 random points inside extent area as pseudo-absence locations
            # (background)
            ...
            # Extract predictor values from the predictor rasters at the pseudo-absence
            # locations
            ...
            # If the current conditions predictor variables list contains 2 or more
            # RasterLayers: Create dataframe with predictor values of each background point
            # Output correlation, histogram and regression plots Output correlation table
            ...
            if (length(arquivo) > 1) {
              ...
              output$grafico_correlacao <- renderPlot({
                pairs(sdmdata, cex = 0.1, fig = TRUE, lower.panel = panel.regression, 
  ...
  datasetInput <- reactive({
    if (exists("especie")) {
      switch("especie", especie = especie)
    }
  })
  ...
  output$mapadistribuicaodatacleaning <- renderLeaflet({
    ...
    if (!is.null(especie)) {
      if (exists("especie")) {
        rownames(especie) <- NULL
        especie$id = 1:nrow(especie)
        # %>% setView(lng = -31.5, lat = -13.4, zoom = 3) map = leaflet(especie) %>%
        # addTiles %>% addCircles(color = 'red', lat = ~ Latitude, lng = ~ Longitude) %>%
        # addMarkers(especie[,1], especie[,2]) %>% addMarkers(especie[,1],
        # especie[,2],popup =~paste('<b><a
        # onclick='document.getElementById(\'edtelemento\').value=',especie[,3],''>ID:
        # ',especie[,3],'</a>') )
        map = leaflet(especie) %>% addTiles %>% addCircles(color = "red", lat = ~Latitude, 
                                                           lng = ~Longitude) %>% addMarkers(clusterOptions = markerClusterOptions()) %>% 
          addMarkers(~Longitude, ~Latitude, popup = ~as.character(id))
        map
      }
    } else {
      showModal(modalDialog(title = "Warning!", "Please inform occurrence data", 
                            easyClose = TRUE))
      ...
      if (input$tipodado == "gbif") {
        ...
        if (input$tipodado == "csv") {
          map = leaflet(especie) %>% addTiles %>% addCircles(color = "red", 
                                                             lat = ~Latitude, lng = ~Longitude) %>% setView(lng = -31.5, lat = -13.4, 
                                                                                                            zoom = 1)
          # %>% addMarkers(especie[,1], especie[,2])
        } else {
          if (input$tipodado == "jabot") {
  ...
  observeEvent(input$btncriarprojeto, {
    ...
    if (projeto != "projeto/") {
      ...
      if (file.exists(paste0(getwd(), "/www/", projeto)) == TRUE) {
        showModal(modalDialog(title = "ERROR!", paste0("This Project Id is already in use!
